...
int function Grenade_OnWeaponToss_( entity weapon, WeaponPrimaryAttackParams attackParams, float directionScale )
{
	...
	if( weapon.HasMod( "flashbang" ) )
		thread FlashbangThreaded( grenade )
	entity weaponOwner = weapon.GetWeaponOwner()
...
void function FlashbangThreaded( entity grenade )
{
	...
	OnThreadEnd(
		function() : ( grenade, fxArray )
		{
			...
			foreach( player in GetPlayerArray() )
				if( IsAlive( player ) )
					FlashPlayer( player, grenade, Distance( player.GetOrigin(), grenade.GetOrigin() ) )
...
void function FlashPlayer( entity player, entity grenade, float distance )
{
	if( player.IsPhaseShifted() )
		return
	if( PlayerCanSee( player, grenade, true, 60 ) )
	{
		ScreenFadeFromColor( player, 255, 255, 255, GraphCapped( distance, 2000, 3000, 255, 191 ), GraphCapped( distance, 1500, 2500, 2.0, 1.0 ), GraphCapped( distance, 0, 2000, 2.0, 0.1 ) )
		thread FlashSound( player, GraphCapped( distance, 0, 2000, 2.0, 0.1 ) )
	}
	else
	{
		TraceResults trace = TraceLine( player.EyePosition(), grenade.GetWorldSpaceCenter(), GetPlayerArray(), TRACE_MASK_BLOCKLOS, TRACE_COLLISION_GROUP_NONE )
		if ( trace.hitEnt == grenade || trace.fraction >= 0.99 )
		{
			if( distance > 1000 )
				return
			ScreenFadeFromColor( player, 255, 255, 255, GraphCapped( distance, 0, 1000, 191, 63 ), GraphCapped( distance, 0, 1000, 2.0, 1.0 ), 0.1 )
			if( distance < 500 )
				thread FlashSound( player, 0.4 )
		}
		else if( distance < 500 )
		{
			ScreenFadeFromColor( player, 255, 255, 255, 127, 1.0, 0.1 )
			thread FlashSound( player, 0.2 )
		}
	}
}

void function FlashSound( entity player, float duration )
{
	player.EndSignal( "OnDeath" )
	player.EndSignal( "OnDestroy" )

	EmitSoundOnEntityOnlyToPlayer( player, player, "quickdeath_fadeout_to_fadein" )
